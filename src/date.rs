...
impl Date {
    ...
    pub fn from_nginx(s: &str) -> Result<Self, DateTimeParseError> {
        if s.len()<11 {
            return Err(DateTimeParseError::UnexpectedEnd);
        }
        if let Ok(year) = s[0..4].parse() {
            ...
            let month = MONTHS_3_LETTERS
                .iter()
                .position(|&m| m == month)
                .ok_or_else(|| DateTimeParseError::UnrecognizedMonth(s.to_owned()))?;
        ...
        match (t.next(), t.next(), t.next()) {
            ...
            (Some(month), Some(day), None) => {
                if let Some(year) = default_year {
                    ...
                    Err(DateTimeParseError::AmbiguousDate(s.to_owned()))
                }
            }
            (Some(day), None, None) => {
